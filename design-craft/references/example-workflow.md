...
This walkthrough shows the expected shape of a Refero Design process. Replace the example
references with actual results from your own research (websearch / browser skills / user-provided references) during real work.
...
Open 3-4 strong styles with `deep review of real reference pages`; full styles are large, so split larger
...
Open the strongest screens with `full inspection of a reference screen`. Use `lookup of comparable screens` if
...
Open relevant flows with `full inspection of a reference flow`.
