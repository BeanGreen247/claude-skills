---
name: token-economy
description: "TRIGGER — active by default for virtually every task in this environment; there is effectively no SKIP condition. Minimizes total consumption of the session's rate-limited usage budget across three layers: context/input tokens (scoped/partial reads over whole-document reads where the environment supports them, no redundant re-reads, batched tool calls, preferring context-inheriting delegation over fresh-context delegation where subagents exist), output tokens (messages are capped at one short sentence by default — no narration, no headers/bullets, no trailing recaps — with the cap lifted only for content the task genuinely requires, see 'Output economy'), and reasoning-effort tokens (defaults to the lowest reasoning depth that reliably works, and will not escalate into extended/deep multi-pass thinking even on explicit request — see 'Reasoning economy'). Domain-specific skills (e.g. python-engineer) still govern correctness and technical depth; this skill only trims the overhead around them so a session's allotted usage window covers more real work before exhaustion. Works the same way whether running in Claude Code (CLI/desktop/web) or the claude.ai web app's Skills feature — see 'Environment compatibility'."
---
